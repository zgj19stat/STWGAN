...
class Trainer():
    ...
    def _critic_train_iteration(self, X, Y, Eta):
        ...
        if self.is_stage1:
            d_real = self.D(torch.concat([X, Y], dim=2).view(X.size(0), -1))
            d_generated = self.D(torch.concat([X, generated_Y], dim=2).view(X.size(0), -1))
            gradient_penalty = self._gradient_penalty(torch.concat([X, Y], dim=2).view(X.size(0), -1))
        else:
    ...
    def _generator_train_iteration_stage1(self, source_X, source_Y, source_Eta, target_X, target_Y, target_Eta):
        ...
        d_source_generated = self.D(torch.concat([source_X, source_generated_Y], dim=2).view(source_X.size(0), -1))
        
        # Get target generated data
        target_Z = self.R(target_X)
        target_generated_Y = self.G(target_Eta, target_Z)
        d_target_generated = self.D(torch.concat([target_X, target_generated_Y], dim=2).view(target_X.size(0), -1))
        ...
        for (source_X, source_Y, source_Eta), (target_X, target_Y, target_Eta) in zip(source_train_loader, target_train_loader):
            ...
            with torch.no_grad():
                if (epoch+1) % 20== 0 and self.num_steps % 50 == 0:
                    ...
                    fake_full_img = torch.cat((vis_X.cpu(), generated_Y.cpu()), dim=2)
