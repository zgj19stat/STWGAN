...
class Trainer():
    ...
    def _valid_epoch(self, epoch, valid_data, valid_Eta):
        ...
        valid_X, valid_Y_mean, valid_Eta = valid_data[0].to(self.device), valid_data[1].to(self.device), valid_Eta.to(self.device)
        ...
        return mse_mean, valid_Y_mean_hat, valid_Y_mean.view(-1)
    ...
    def train(self, train_data, valid_data, valid_Eta, test_data, test_Eta, epochs, args):
        ...
        latest_mse_mean = torch.tensor(0.)
        ...
        for epoch in range(epochs):
            ...
            if not args.is_test:
                mse_mean, valid_Y_mean_hat, valid_Y_mean = self._valid_epoch(epoch, valid_data, valid_Eta)

                mse_mean_record[epoch] = mse_mean
                if epoch>=20:
                    latest_mse_mean = mse_mean_record[epoch-20:epoch].mean()
                    print("latest mse mean: {}".format(latest_mse_mean))
                    
                if self.is_stage1:
                    if self.target_only:
                        with open(args.result + '/training_record.csv', mode='a', newline='') as file:
                            writer = csv.writer(file)
                            writer.writerow([epoch+1,latest_mse_mean.item(), mse_mean.item(), 
        ...
        # test data
        if args.is_test:
            test_mse_mean, _, _ = self._valid_epoch(epoch, test_data, test_Eta)
            if self.is_stage1 and (self.target_only or args.MSSG):
                with open(args.result + '/training_record.csv', mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([test_mse_mean.item()])
            elif not self.is_stage1:
                with open(args.result + '/training_record2.csv', mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([test_mse_mean.item()])
                
        return mse_mean_record
